...
public class Tema extends Tema_ventana implements View {
	...
	public void crearMensaje() {
		bd.crearMensaje(contenido.getValue(), idu, id, 1);
	...
	public void gustarTema() {
		new Vista_tema_heredar().like_html.click();
	}
	
	public basededatos.Mensaje[] mensajesTema() {
		try {
			return TemaDAO.getTemaByORMID(id).tiene.toArray();
		} catch (PersistentException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public void borrarTema() {
		new Vista_tema_heredar().borrar_html.click();
	}
}